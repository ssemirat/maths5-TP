...
Plus généralement, si chaque observation est indépendante des autres, alors la \emph{vraisemblance} de la loi $P$, donnée par $$L(P)=\prod\limits_{j:Y_j=1} P(Y=1|X_j)\prod\limits_{j:Y_j=0}(1-P(Y=1|X_j))$$ est la probabilité d'observer $Y=Y_1$ étant donné $X_1$, \textit{et} $Y=Y_2$ étant donné $X_2$, \textit{et}, \ldots, \textit{et} $Y=Y_J$ étant donné $X_J$. 
C'est la probabilité d'observer les données effectivement observées.
